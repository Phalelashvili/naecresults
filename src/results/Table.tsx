...
export default function DataTable({
  ...
  const renderCell = useCallback((row: Row, col: ColumnDef, isRowFav: boolean) => {
    ...
    if (col.key === "progName") {
      ...
      return (
        <span className="flex items-center gap-1 group/prog">
          ...
          <Link
            to={`/results/${year}/program/${row.progCode}`}
    ...
    if (col.key === "uniName") {
      ...
      return (
        <span className="flex items-center gap-1 group/uni">
          ...
          <Link
            to={`/results/${year}/uni/${row.uniCode}`}
    ...
    // merged "raw → scaled" score cell — raw prominent, scaled muted
    const RAW_OF: Partial<Record<keyof Row, keyof Row>> = {
      geoScaled: "geoRaw", langScaled: "langRaw", sub1Scaled: "sub1Raw", sub2Scaled: "sub2Raw",
    };
    if (col.key in RAW_OF) {
      const raw = row[RAW_OF[col.key]!];
      if (raw == null && val == null) return <span className="text-gray-300">&mdash;</span>;
      return (
  ...
  return (
    <div className="h-full overflow-x-auto">
      <div style={{ minWidth: totalWidth }} className="h-full flex flex-col">
        <div className="rt-head sticky top-0 z-20 flex bg-gray-100 border-b border-gray-300 text-xs font-semibold text-gray-600 uppercase tracking-wider shrink-0">
          {onAddToList && (
